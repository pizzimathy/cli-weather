...
Output.prototype.warnings = function () {
    ...
    if (this.alerts.length != 0) {
        ...
        for (var alert in this.alerts) {
            ...
            if (this.alerts.hasOwnProperty(alert)) {
            
                warn = this.alerts[alert];
                color = this._assignWarningColor(warn.title);
            
                warnstring += chalk.bold("Alert: " + warn.title);
                warnstring += chalk.bold(" active from " + realTime(warn.time, true) + " to " + realTime(warn.expires, true));
                warnstring = chalk[color](warnstring);
            
                console.log(warnstring);
            }
        }
    
        console.log(chalk.bgRed("Please visit alerts.weather.gov for more information."));
        console.log();
    }
};
