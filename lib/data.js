...
function collectHighs(data, units) {
    ...
    data.forEach(function (obj) {
        ...
        temp = 'high: ' + (point > 32 ? chalk.red(point.toString()) : chalk.blue(point.toString()) + units.tmp);
        highs.push(temp);
    });
...
function collectLows(data, units) {
    ...
    data.forEach(function (obj) {
        ...
        temp = 'low: ' +(point > 32 ? chalk.red(point.toString()) : chalk.blue(point.toString()) + units.tmp);
        lows.push(temp);
    });
...
function display(now, units) {
    ...
    var temp = (now.temperature > 32 ? chalk.red(Math.ceil(now.temperature)) : chalk.blue(Math.floor(now.temperature))) + units.tmp,
