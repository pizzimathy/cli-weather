...
function collectHighs(data) {
    ...
    data.forEach(function (obj) {
        var temp = Math.floor(obj.temperatureMax);

        highs.push(
            'high: ' +
            (temp >= 32 ? red(temp.toString() + '˚') : blue(temp.toString() + '˚'))
        )
    });
...
function collectLows(data) {
    ...
    data.forEach(function (obj) {
        var temp = Math.floor(obj.temperatureMin);
...
function display(now) {
    ...
    current += (now.temperature >= 32 ? clc.red(Math.floor(now.temperature) + '˚') : clc.blue(Math.floor(now.temperature) + '˚')) +
