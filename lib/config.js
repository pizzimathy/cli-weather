...
function write(args, units, ip) {
    fs.writeFile('./lib/config.json', JSON.stringify(args), function (err) {
        if (err) {
...
function read(units, ip) {
    fs.readFile('./lib/config.json', function (err, data) {
        if (err) {
            console.log(clc.red('✗ couldn\'t read preset data'));
            console.log(err);
        } else {
